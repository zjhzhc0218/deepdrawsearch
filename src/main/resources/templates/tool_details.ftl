			...
			<div class="col-md-8">
				<div class="toolDetails_con">
					...
					<div class="toolDetails_go">
						<p>上一篇：<a href="##">比利时争做eWTP标杆，将建欧洲首个ehub</a></p>
						<p>下一篇：<a href="##">蒙牛拥抱天猫新零售</a></p>
					</div>
				</div>
			</div>
			<div class="col-md-4">
				<div class="hot_news">
					...
					<ul>
						<li v-for="(item,index) in hotnews"><a :href="item.link"><b class="num" :class="index<3?'numFir':''">{{item.num1}}</b><span class="text">{{item.title}}</span><span class="date">{{item.date}}</span></a></li>
					</ul>
				</div>
